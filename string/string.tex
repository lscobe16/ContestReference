...
\begin{algorithm}{\textsc{Knuth-Morris-Pratt}-Algorithmus}
	% TODO: wozu, wenn wir p und z Funktion haben? Z(P#T) == |P| gibt uns auch alle Matches
	\begin{methods}
...
\begin{algorithm}{Z- und P-Funktion}
	\begin{methods}[ll]
	...
	\begin{methods}[ll]
		$p_i\coloneqq$ Längstes echtes Präfix von $s_0\cdots s_i$ das auch Suffix davon ist  ($p_0=0$)& \runtime{n}
	\end{methods}
	\sourcecode{string/p.cpp}
\end{algorithm}

\begin{algorithm}{Rolling Hash}
	$\textbf{P}[$Kollsion bei $\sqrt{M}$ strings$]\approx \frac{1}{2}$ \quad (birthday paradox)\\
	Für mehr Sicherheit: größeres M, mehrere Hashes (M's)
	%TODO: dokumentieren, wie das ist, wenn wir nicht uniform randomness annehmen wollen (siehe FAP-Folien)
...
\begin{algorithm}{\textsc{Manacher}'s Algorithm, Longest Palindrome}
	\begin{methods}
		\method{manacher}{Längen der Palindrome mit Zentrum bei Buchstaben i/2 und in der Lücke danach}{n}
	\end{methods}
...
\begin{algorithm}[optional]{Lyndon}
	\begin{itemize}
...
\end{methods}
\textbf{ACHTUNG:} \code{s} muss mit einem Sentinel enden! \code{'\$' <'A' <'a' <'~'}
% Vielleicht auch nicht mehr, aber schadet auch nicht. Man kann dadurch auch aussuchen, ob Präfixe kleiner oder größer sind
