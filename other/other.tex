...
\begin{algorithm}{Bit Operations}
	...
	\begin{tabularx}{\linewidth}{|Ll|}
		...
		Anzahl an gesetzten bits & \code{__builtin_popcountll(x)} \\
		$i$-te Zahl eines Graycodes & \code{i ^ (i >> 1)} \\
		Niederwertigstes Bit in x & \code{x & -x} \\
		\hline
	\end{tabularx}\\
	\end{expandtable}
...
\begin{algorithm}{Koordinatenkompression}
	\sourcecode{other/coordCompression.cpp}
\end{algorithm}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Wenn vorher \code{cin >> ...} benutzt, dann zuerst Newline mit \code{getline(cin, str)} fressen.
		\item Zeilen weiter aufteilen: \code{stringstream(str) >> ...}
		\item Bis zum Ende einlesen: \code{while(cin >> a >> b >> ...)}
	\end{itemize}
...
\subsection{ATIS VS Code}

Die C/C++ Extension frisst Speicher, den die ATIS nicht gönnt.
\begin{itemize}
	\item C/C++ Extension installieren (nicht Pack)
	\item In VS Code Einstellungen nach \code{cache} suchen
	\item Einstellung \code{C_Cpp: Intelli Sense Cache Size} auf $128$ stellen (oder zumindest $\leq 512$)
	\item Wenn schon akut: Ordner \codeSafe{~/.cache/vscode-cpptools} löschen (existiert erst wenn man mit Extension gearbeitet hat)
\end{itemize}
