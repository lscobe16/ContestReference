struct segT {
    ...
    @\orange{shared\_ptr<vector<node\\>\\> v\{new vector<node>(\{0, 0, e, id\})\};}@ //Array wegen Speicher (int32_t < node*). Is that worth it???
...
#define N v[n]
    ...
    z bin_search(z a, z bex, z l, z rex, z n, A &cur_agg, U lazy, function<bool(A&)> &p@\opt{, bool rtl}@) { @\optAnn@
        ...
        A new_agg = @\opt{rtl ? aggr(n\_agg, cur\_agg) : }@aggr(cur_agg, n_agg);
