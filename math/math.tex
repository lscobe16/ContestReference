...
\begin{algorithm}{Zykel Erkennung}
	\begin{methods}
		\method{cycleDetection}{findet Zyklus in $f$-Successor-Graph von $x_0$ aus}{startI+len}
	\end{methods}
	\sourcecode{math/cycleDetection.cpp}
\end{algorithm}
...
\method{modmul}{berechnet $a \cdot b \bmod M$}{1}
\method{modpow}{berechnet $b^e \bmod M$}{\log(e)}
	\sourcecode{math/modMulPow.cpp}
\begin{itemize}
	\item modmul ca. 1,3x - 2x schneller als \code{__int128}
	\item für $a > 10^9$ \code{__int128} oder \code{modmul} benutzten!
\end{itemize}

\begin{algorithm}{ggT, kgV, erweiterter euklidischer Algorithmus}
	\code{gcd} und \code{lcm} aus \code{std} verwenden.\\
	\runtime{\log(a) + \log(b)}
	% \sourcecode{math/gcd-lcm.cpp} % std is equivalent (also in time)
	\code{gcd} and \code{lcm} in \code{std}-Library
...
% TODO: get new Transforms from Yidi
...
\method{gauss}{gaußt die Matrix (wenn möglich) zur Einheitsmatrix}{n^3}
\method{invert}{invertiert eine quadratische Matrix in-place}{n^3}
...
\paragraph{Binomialkoeffizienten}
	...
	\begin{methods}
		\method{}{berechne viele Binomialkoeffizienten vor}{N^2}
	\end{methods}
	\sourcecode{math/binomial_DP.cpp}
